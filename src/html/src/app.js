...
export class App extends LitElement {
    ...
    buildRouteContent(route) {
        switch (route) {
            ...
            case 'custom-mixer':
                return html`<custom-mixer-panel></custom-mixer-panel>`
            case 'bluepad':
                return elrsState.config.bluepad !== undefined ? html`<bluepad-panel></bluepad-panel>` : null
            // FEATURE:NOT IS_TX
            // FEATURE:NOT IS_8285
            case 'am32':
                return html`<am32-panel></am32-panel>`
            // /FEATURE:NOT IS_8285
            case 'vesc':
                return html`<vesc-panel></vesc-panel>`
            // /FEATURE:NOT IS_TX
            case 'models':
    ...
    ensureLoadedForRoute(route) {
        ...
        if (['hardware', 'custom-mixer', 'bluepad', 'voltage', 'am32', 'vesc', 'lr1121'].includes(route)) {
